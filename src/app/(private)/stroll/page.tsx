...
export default function StrollPage() {
  ...
  const progress = destination && destination.meter > 0
    ...
    : 0;
  const mapSource = destination && location
    ? `/api/map?${new URLSearchParams({
        originLat: String(location.latitude),
        originLng: String(location.longitude),
